...
export default function Breadcrumbs({ items }: { items: BreadcrumbItem[] }) {
  ...
  return (
    <>
      ...
      >
        <ol className="flex flex-wrap items-center gap-1.5 text-xs font-barlow text-white/80">
          {items.map((item, i) => {
            ...
            return (
              <li key={`${item.name}-${i}`} className="flex items-center gap-1.5">
                {item.path && !isLast ? (
                  <Link
                    href={item.path}
                    className="inline-flex items-center min-h-[24px] hover:text-ecm-lime transition-colors"
                  >
                    {item.name}
                  </Link>
                ) : (
                  <span
                    className={isLast ? "text-ecm-lime" : "text-white/80"}
